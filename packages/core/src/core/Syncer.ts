...
export class Syncer {
  ...
  constructor(target: HTMLElement, renderer: Renderer, extractorConfig?: ExtractorConfig) {
    this.target = target;
  ...
  private forceUpdateScene() {
    ...
    const sceneGraph = extractSceneGraph(this.target, this.pendingMask, this.extractorConfig);
